...
schema = StructType([
    ...
    StructField("tag_id", StringType(), nullable=True),
    StructField("x_position", DoubleType(), nullable=True),
    StructField("y_position", DoubleType(), nullable=True),
    StructField("timestamp", TimestampType(), nullable=True),
    StructField("anchor_info", StringType(), nullable=True)  # JSON 문자열로 처리
])
...
try:
    kafkaDF = spark.readStream \
        ...
        .option("kafka.group.id", "spark-append") \
        .load()
except Exception as e:
...
delta_table_path = f"s3a://{bucket_name}/spark-stream-table"
checkpoint_path = f"s3a://{bucket_name}/spark-stream-table/checkpoints"
